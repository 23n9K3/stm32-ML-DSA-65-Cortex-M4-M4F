...
extern void KeccakF1600_StateXORBytes(void *state, const uint8_t *data,
                                      unsigned int offset,
                                      unsigned int length);
extern void KeccakF1600_StateExtractBytes(const void *state, uint8_t *data,
                                          unsigned int offset,
                                          unsigned int length);
...
static void keccak_state_xor_bytes(uint64_t *state, const uint8_t *data,
                                   size_t offset, size_t length) {
#if MLDSA_USE_KECCAK_ASM
    KeccakF1600_StateXORBytes(state, data, (unsigned int)offset,
                              (unsigned int)length);
#else
    size_t i;
    for (i = 0; i < length; ++i) {
        size_t pos = offset + i;
        state[pos >> 3] ^= (uint64_t)data[i] << (8U * (pos & 7U));
    }
#endif
}

static void keccak_state_extract_bytes(const uint64_t *state, uint8_t *data,
                                       size_t offset, size_t length) {
#if MLDSA_USE_KECCAK_ASM
    KeccakF1600_StateExtractBytes(state, data, (unsigned int)offset,
                                  (unsigned int)length);
#else
    size_t i;
    for (i = 0; i < length; ++i) {
        size_t pos = offset + i;
        data[i] = (uint8_t)(state[pos >> 3] >> (8U * (pos & 7U)));
    }
#endif
}
...
static void keccak_absorb(uint64_t *s, uint32_t r, const uint8_t *m,
    ...
    /* Zero state */
    memset(s, 0, 25U * sizeof(uint64_t));

    while (mlen >= r) {
        keccak_state_xor_bytes(s, m, 0, r);
        KeccakF1600_StatePermute(s);
        mlen -= r;
        m += r;
    }

    keccak_state_xor_bytes(s, m, 0, mlen);
    pad = p;
    keccak_state_xor_bytes(s, &pad, mlen, 1);
    pad = 0x80U;
    keccak_state_xor_bytes(s, &pad, r - 1U, 1);
}
...
static void keccak_squeezeblocks(uint8_t *h, size_t nblocks,
    ...
    while (nblocks > 0) {
        KeccakF1600_StatePermute(s);
        keccak_state_extract_bytes(s, h, 0, r);
        h += r;
        nblocks--;
    }
}
...
static void keccak_inc_absorb(uint64_t *s_inc, uint32_t r, const uint8_t *m,
    ...
    while (mlen + s_inc[25] >= r) {
        take = (size_t)r - (size_t)s_inc[25];
        keccak_state_xor_bytes(s_inc, m, (size_t)s_inc[25], take);
        mlen -= take;
        m += take;
        s_inc[25] = 0;
    ...
    keccak_state_xor_bytes(s_inc, m, (size_t)s_inc[25], mlen);
    s_inc[25] += mlen;
}
...
static void keccak_inc_finalize(uint64_t *s_inc, uint32_t r, uint8_t p) {
    ...
    pad = p;
    keccak_state_xor_bytes(s_inc, &pad, (size_t)s_inc[25], 1);
    pad = 0x80U;
    keccak_state_xor_bytes(s_inc, &pad, r - 1U, 1);
    s_inc[25] = 0;
}
...
static void keccak_inc_squeeze(uint8_t *h, size_t outlen,
    ...
    i = outlen < s_inc[25] ? outlen : (size_t)s_inc[25];
    keccak_state_extract_bytes(s_inc, h, r - (size_t)s_inc[25], i);
    h += i;
    outlen -= i;
    s_inc[25] -= i;
    ...
    while (outlen > 0) {
        ...
        i = outlen < r ? outlen : r;
        keccak_state_extract_bytes(s_inc, h, 0, i);
        h += i;
        outlen -= i;
        s_inc[25] = r - i;
    }
